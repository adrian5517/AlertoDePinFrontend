...
const DashboardPolice = () => {
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        ...
        // assuming addNotification(message, type)
  ...
  useEffect(() => {
    ...
    // Real-time notifications (from backend Notification model)
    socket.on('newNotification', (notif) => {
      ...
      // Always provide title and message for NotificationPanel
      addNotification({
  ...
  const stats = [
    ...
    { label: 'Response Rate', value: '94%', icon: Shield, color: 'bg-blue-600' }
  ];
  ...
  const handleRespond = async (alertId) => {
    try {
      ...
      setAlerts((prev) =>
        prev.map((a) =>
          (a._id || a.id) === alertId
            ? { ...a, status: 'responded', responder: user }
            : a
        )
      );
  ...
  const handleMarkArrived = async (alertId) => {
    try {
      await alertsAPI.update(alertId, {
        status: 'active',
        notes: 'Responder has arrived on scene'
      });

      setAlerts((prev) =>
        prev.map((a) =>
          (a._id || a.id) === alertId
            ? { ...a, status: 'active', arrivedAt: new Date() }
            : a
        )
      );
  ...
  const handleResolve = async (notes) => {
    try {
      ...
      if (selectedAlert?._id === alertId || selectedAlert?.id === alertId) {
  ...
  return (
    <div className="space-y-6">
      <DashboardHeader
        ...
        subtitle={`Officer ${user?.name} - Real-time incident monitoring`}
        stats={stats}
      />
              ...
              {alerts.map((alert) => (
                ...
                >
                  ...
                  <div className="space-y-2">
                    {/* Pending/Active - Show Respond button */}
                    {(alert.status === 'pending' || alert.status === 'active') &&
                      alert.responder?._id !== user?._id && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setModalAlertId(alert._id || alert.id);
                            setShowRespondModal(true);
                          }}
                          className="w-full text-xs font-semibold bg-blue-600 hover:bg-blue-700 text-white py-2 px-3 rounded-lg transition-colors flex items-center justify-center gap-1"
                        >
                    ...
                    {/* Responded - Show Arrived and Resolve buttons */}
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <button
                              ...
                              className="text-xs font-semibold bg-green-600 hover:bg-green-700 text-white py-2 px-3 rounded-lg transition-colors flex items-center justify-center gap-1"
                            >
                            ...
                            <button
                              ...
                              className="text-xs font-semibold bg-purple-600 hover:bg-purple-700 text-white py-2 px-3 rounded-lg transition-colors flex items-center justify-center gap-1"
                            >
                              <CheckCircle className="w-3 h-3" />
                              Resolve
                            </button>
                    ...
                    {/* Active with you as responder - Show Resolve */}
                    {alert.status === 'active' &&
                      alert.responder?._id === user?._id && (
                        <>
                          <div className="p-2 bg-green-50 dark:bg-green-900/20 rounded text-xs">
                            <span className="text-green-700 dark:text-green-300 font-semibold flex items-center gap-1">
                              <CheckCircle className="w-3 h-3" />
                              On scene
                            </span>
                          </div>
                          <button
                            ...
                            className="w-full text-xs font-semibold bg-purple-600 hover:bg-purple-700 text-white py-2 px-3 rounded-lg transition-colors flex items-center justify-center gap-1"
                          >
                            <CheckCircle className="w-3 h-3" />
                            Mark as Resolved
                          </button>
      ...
      <ConfirmModal
        ...
        onConfirm={() => handleRespond(modalAlertId)}
        title="Respond to Incident"
        message="Are you sure you want to respond to this incident? This will notify the reporter that you are on the way."
        type="confirm"
      />
      ...
      <ConfirmModal
        ...
        onConfirm={() => handleMarkArrived(modalAlertId)}
        title="Confirm Arrival"
        message="Confirm that you have arrived at the incident location. This will notify the reporter that you are on scene."
        type="success"
      />
      ...
      <ResolveModal
        ...
        alert={alerts.find((a) => (a._id || a.id) === modalAlertId)}
      />
    </div>
  );
};
